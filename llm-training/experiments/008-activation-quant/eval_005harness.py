"""Evaluate the 007 QAT-ternary checkpoint on 005's 20-fixed-batch harness.

Places 007 on the exact same measurement axis as the 66.6 (FP32) / 627 (PTQ
ternary) numbers: same val split (last 10% of tinyshakespeare through 002's
tokenizer), same fixed batches (seed 1234, 20 batches, batch_size 16).

The 007 checkpoint's model_config has quantize_linears=True, so reconstructing
the model rebuilds BitLinear -- the fake-quant happens inside forward(). This is
a plain forward pass: NO apply_ptq step (unlike 005, which PTQs the FP32 002
checkpoint post-hoc).
...
CKPT = Path("checkpoints/007-qat-ternary")  # pull from S3 to here to reproduce
...
loss = val_loss(model)
print(f"\n007 QAT-ternary (005 harness): val loss {loss:.4f}   ppl {math.exp(loss):.1f}")
